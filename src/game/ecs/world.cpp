...
namespace wf::game::ecs {
    ...
    void World::RegistrySystem() const {
        ...
        world->system<const Gravity, const Mass, Position, Velocity>("GravitySystem").kind(flecs::OnUpdate).each(GravitySystem);
...
//        world->system<Velocity, ContinuousAcceleration, ImpulseAcceleration>("AccelerationSystem").kind(flecs::OnUpdate).each(AccelerateSystem);
        world->system<Velocity, const ImpulseAcceleration>("AccelerationSystem").kind(flecs::OnUpdate).each(AccelerateSystem);
        world->system<Velocity, const Drag>("DragSystem").kind(flecs::OnUpdate).each(DragSystem);
        world->system<Position, Velocity>("CollisionSystem").kind(flecs::OnUpdate).each(CollisionSystem);
